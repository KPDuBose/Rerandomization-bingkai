...
# Varied threshold and sample sizes
###################
rm(list = ls())
set.seed(123)
library(tidyverse)
...
# Sample size - varied
n_grid <- c(30, 100, 250, 500, 1000)
pi <- 0.5
n_sim <- 1
# Get threshold to vary
accept_grid <- c(0.40, 0.20, 0.10, 0.01)
tt_grid <- qchisq(accept_grid, df = 2)
...
# Set up vector for all results
all_results <- vector("list", length = nrow(param_grid))
...
for (g in seq_len(nrow(param_grid))){
  ...
  message(
    "Running scenario ", g, " of ", nrow(param_grid),
    ": n = ", n,
    ", tt = ", round(tt, 2)
  )
...
saveRDS(all_results, file = "simulation/sim1-3.rds")

# summarizing results

results <- readRDS(file = "simulation/sim1-3.rds")
n_sim <- ncol(results)
tt <- 1
n <- 400
